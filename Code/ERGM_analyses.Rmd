...
plant.traits <- read.csv("Data/Plant_traits/all_plant_traits.csv", sep=",")
...
flowers <- read.csv("Data/synchrony_data_plant_sp.csv")[ ,1:6]
...
# using another flowering synchrony index (qualitative)
...
plant.xy <- read.csv("Data/coordinates_individual_plants.csv", sep=";") %>% filter(Plant_id %in% c(int.mat$Plant_id))
...
mean.A <- uni.mat.plot[[1]] %>% mutate(count=rowSums(.!=0), sum=rowSums(.)) %>% select(count, sum) %>% summarise(mean_count=mean(count), mean_sum=mean(sum))
mean.B <- uni.mat.plot[[2]] %>% mutate(count=rowSums(.!=0), sum=rowSums(.)) %>% select(count, sum) %>% summarise(mean_count=mean(count), mean_sum=mean(sum))
mean.C <- uni.mat.plot[[3]] %>% mutate(count=rowSums(.!=0), sum=rowSums(.)) %>% select(count, sum) %>% summarise(mean_count=mean(count), mean_sum=mean(sum))
mean.D <- uni.mat.plot[[4]] %>% mutate(count=rowSums(.!=0), sum=rowSums(.)) %>% select(count, sum) %>% summarise(mean_count=mean(count), mean_sum=mean(sum))
mean.E <- uni.mat.plot[[5]] %>% mutate(count=rowSums(.!=0), sum=rowSums(.)) %>% select(count, sum) %>% summarise(mean_count=mean(count), mean_sum=mean(sum))
mean.F <- uni.mat.plot[[6]] %>% mutate(count=rowSums(.!=0), sum=rowSums(.)) %>% select(count, sum) %>% summarise(mean_count=mean(count), mean_sum=mean(sum))
...
uni <- round((uni.mat.plot[[1]]) *100, 0)
...
network::set.network.attribute(uni.net, "pheno", as.matrix(pheno)) # spatial distance between plants as network attribute
...
uni <- round((uni.mat.plot[[2]]) *100, 0)
uni %<>% select(order(colnames(uni))) %>% as.matrix()
...
#eff.all <- rbind(effA, effB, effC, effD,effE, effF)
...
#eff.all %<>% mutate(Estimate=ifelse(!is.finite(Estimate), -3, Estimate))
...
ggplot(eff.all, aes(x=predictor, y=Estimate, fill=method, group=method)) +
  ...
  facet_wrap(~plot, nrow=2, scales="free_x", labeller = labeller(plot = plot.labs)) + 
...
table.output <-cbind(coef(summary(uni.ergm.fitA.b)),
...
# Heterospecific vs conspecific pollinator sharing probabilities
...
b.plot <- ggplot(predict.all.sum, aes(x=(n_pol), y=(mean), color=Plant_sp)) + 
  scale_x_continuous(labels=scaleFUN) + 
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) + 
  geom_point(alpha=0.5) + 
  geom_smooth(method = "lm", se = F) + 
  facet_grid(het_con ~ Plant_sp, 
  labeller = labeller(Plant_sp = sp.labs, het_con= sh.labs), scales="free_x") +
  scale_color_manual(values=alpha(cols2, 0.8)) + theme_bw() + theme(
   strip.background = element_rect(color="white", fill="white", size=1.5), 
   ...
   axis.title.y = element_text(face = "bold", size=12, margin = margin(t = 0, r = 10, b = 0, l = 0)),
   legend.position = "none") +
  xlab("Number of pollinator species") + ylab("Probability of sharing \n pollinators with more plants")
...
w.plot <- ggplot(sim.all.sum, aes(x=(n_pol), y=(mean), color=Plant_sp)) + 
  scale_x_continuous(labels=scaleFUN) + 
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) + 
  geom_point(alpha=0.5) + 
  geom_smooth(method = "lm", se = F) + 
  ...
  scale_color_manual(values=alpha(cols2, 0.8)) + theme_bw() + theme(
   ...
   axis.title.y = element_text(face = "bold", size=12, margin = margin(t = 0, r = 10, b = 0, l = 0)),
   legend.position = "none") +
  xlab("Number of pollinator species") + ylab("Probability of sharing more \n pollinators with other plants")


ggarrange(b.plot, w.plot, ncol = 1, nrow = 2)


# het vs con (binary and weighted)
...
b.w.plot <- ggarrange(b.plot, w.plot, ncol = 1, nrow = 2)
...
# Heterospecific vs conspecific sharing probabilities
...
#ggarrange(het.con.plot, b.w.plot, ncol = 1, nrow = 2)
...
uni <- round((uni.mat.plot[[6]]) *100, 0)
#uni %<>% select(order(colnames(uni))) %>% as.matrix()
#temp <- int.mat %>% column_to_rownames("Plant_id") %>% select_if(is.numeric) 
#uni <- as.data.frame(as.matrix(1-vegdist(bi.mat.plot[[1]], method = "bray")))*100
...
str(plant.int)
#plant_id_sp$Plant_species <- as.factor(plant_id_sp$Plant_species)
...
#l <- layout.graphopt(bipartite.web)
